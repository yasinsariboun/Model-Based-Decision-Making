...
def get_model_for_problem_formulation(problem_formulation_id):
    ...
    '''
    # Load the model:
    function = DikeNetwork() #DikeNetwork is the function we will work with same as lake model function
    # workbench model:
    ...
    if problem_formulation_id == 0:
        ...
        for n in function.planning_steps: #planning steps is an array of numbers 0 -> x
    ...
    # 3-objectives PF:
        ...
        for n in function.planning_steps:
            ...
            dike_model.outcomes = [
                            ScalarOutcome(['Expected Annual Damage {}'.format(dike,n) for dike in function.dikelist] ,
                                   kind=direction),

            ScalarOutcome(['Total Investment Costs {}'.format(dike,n) for dike in function.dikelist],
                           kind=direction),

            ScalarOutcome(['Expected Number of Deaths {}'.format(dike,n) for dike in function.dikelist],
                           kind=direction)]
    ...
    # Fully disaggregated:
        ...
        for n in function.planning_steps:
            ...
            variable_names__.extend(['{}_Expected Number of Deaths {}'.format(dike, n)
                                     for dike in function.dikelist])
        #print(variable_names_)
        for n in function.planning_steps:
            variable_names_=['{}_Dike Investment Costs {}'.format(dike, n)
                                    for dike in function.dikelist] + [
                                       'RfR Total Costs {}'.format(n)
                                   ] + ['Expected Evacuation Costs {}'.format(n)]
            print(variable_names_)
            for dike in function.dikelist:
                ...
                outcomes.append(ScalarOutcome('Total Investment Costs {}'.format(n),
                                              variable_name=[var for var in variable_names_],
                                              function=sum_over,
                                               kind=direction))
